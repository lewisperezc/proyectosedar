 ...
 function validar_vacios()
  {
		...
		if(form.mes.selectedIndex == 0){ CamposVacios += "* Mes\n"; form.mes.focus(); }
			...
			form.action = '../reportes_PDF/mayor_y_balance.php';
			form.submit();
  }
  </script>
</head>
...
?>
<form name="mayor_y_balance" id="mayor_y_balance" method="post">
	<table border="2" bordercolor="#0099CC">
    	<tr>
        	<th colspan="4">Mayor Y Balance</th>
        </tr>
        <tr>
        	<th>Desde</th><td><input type="text" name="may_desde" id="may_desde" onkeypress="return permite(event,'num')"/></td>
            <th>Hasta</th><td><input type="text" name="may_hasta" id="may_hasta" onkeypress="return permite(event,'num')"/></td>
        </tr>
        <tr>
            <th>Mes</th>
