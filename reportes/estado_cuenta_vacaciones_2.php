...
//$cue_vacaciones='23809505';//ANTIGUO PUCH
//$sql="EXECUTE estado_cuenta_vacaciones '$cedula','$cue_vacaciones'";

$cue_vacaciones='25051005';//NUEVO PUCH
$sql="EXECUTE estado_cuenta_vacaciones $doc_ini,'$cue_vacaciones'";
//echo $sql;
...
if($query)
{
	...
	$html.='<tr>';
	$html.='<th colspan="6">ESTADO DE CUENTA FONDO DE VACACIONES</th>';
	$html.='</tr>';
	...
	$query="SELECT * from movimientos_contables where mov_nit_tercero='$dat_asociados[nit_id]' and
	mov_cuent='$cue_vacaciones' and mov_compro like('CIE-2017')";
	//echo $query;
    $ejecutar=mssql_query($query);
    $saldo_inicial=mssql_fetch_array($ejecutar);
	
	$html.='<th>SALDO INICIAL:</th><td style="text-align:center;">'.number_format($saldo_inicial["mov_valor"]).'</td>';
	$html.='</tr>';
	...
	{	
		if($tempciclo==0)
		{
			...
			if($temp==0)
			{
				...
				if($row["trece"]==1)
				{
					$saldo_mes=$saldo_inicial["mov_valor"]-$row["dos"];
...
echo '<script>location.href="../reportes_PDF/estado_cuenta_vacaciones.php";</script>';
?>
