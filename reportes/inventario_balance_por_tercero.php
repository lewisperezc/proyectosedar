...
<title>Inventario Y Balance Por Tercero</title>
<script src="../librerias/js/validacion_num_letras.js"></script>
<script>
 function validar_vacios()
  {
		var form = document.inventario_balance_por_tercero;
		...
		if(form.mes.selectedIndex == 0){ CamposVacios += "* Mes\n"; form.mes.focus(); }
			...
			form.action = '../reportes_PDF/inventario_balance_por_tercero.php';
			form.submit();
  }
  </script>
</head>
...
?>
<form name="inventario_balance_por_tercero" id="inventario_balance_por_tercero" method="post">
	<table border="2" bordercolor="#0099CC">
    	<tr>
        	<th colspan="4">Inventario Y Balance Por Tercero</th>
        </tr>
        <tr>
        	<th>Desde</th><td><input type="text" name="inv_desde" id="inv_desde" onkeypress="return permite(event,'num')"/></td>
            <th>Hasta</th><td><input type="text" name="inv_hasta" id="inv_hasta" onkeypress="return permite(event,'num')"/></td>
        </tr>
        <tr>
            <th>Mes</th>
            <td><select name="mes" id="mes"><option value="0">Seleccione</option>
