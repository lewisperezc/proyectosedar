...
$fec_ini = "01-"."0".$mes."-".$ano;
$fec_fin = $ult_dia."-"."0".$mes."-".$ano;
$totcargeneral=0;
$pdf =& new Cezpdf('a4');
...
if($query)
{
	//$sql_rep = "SELECT * FROM reportes ORDER BY siete,CAST(cinco AS INT)";
	//$sql_rep = "SELECT * FROM reportes WHERE ISNULL(CAST(cuatro AS INT),0)-ISNULL(CAST(diez AS INT),0)>0 ORDER BY siete,CAST(dos AS DATETIME),cinco";
	...
	while($row=mssql_fetch_array($query_rep))
	{
	  ...
	  if($p==0)
	  {
        ...
        /*Dias cartera*/$pdf->SetY(40+($i*3));$pdf->SetX(130);$pdf->Cell(21,8,$rotacion_cartera);
                //echo "los dias: ".$dias_factura."___".$dias."<br>";
        //$dias=192;
