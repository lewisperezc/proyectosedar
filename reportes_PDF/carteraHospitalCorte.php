...
$ult_dia = date("d",(mktime(0,0,0,$mes_fin+1,1,$ano_fin)-1));
$fec_ini = "01-".$mes_ini."-".$ano_ini;
$fec_fin = $ult_dia."-".$mes_fin."-".$ano_fin;
$totcargeneral=0;
$pdf =& new Cezpdf('a4');
...
$fec_dia=date('d-m-Y');
$pdf->SetY(26);$pdf->SetX(223);$pdf->Cell(21,8,$fec_fin);
...
if($query)
{
	...
	if($mes_fin<=9)
		...
		$mes_fin_con=$mes_fin;
	//$sql_rep = "SELECT * FROM reportes WHERE CAST(dos AS DATETIME) BETWEEN '01-".$mes_ini_con."-".$ano_ini."' AND '".$ult_dia."-".$mes_fin_con."-".$ano_fin."'
//ORDER BY siete,cinco";
$sql_rep="SELECT uno,dos,tres,cinco,seis,siete,ocho,nueve,once,doce,trece,catorce,quince,diesiseis,diesisiete,diesiocho,
	...
	while($row=mssql_fetch_array($query_rep))
	{
	  ...
	  if($p==0)
	  {
		...
		$rotacion_cartera=$dias_factura+$dias;
		
        /*Dias cartera*/$pdf->SetY(40+($i*3));$pdf->SetX(130);$pdf->Cell(21,8,$rotacion_cartera);
