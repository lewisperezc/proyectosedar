...
/*
$con_dat_cue_fabs=$ins_fabs->ConTodPorFonFabs();
while($res_dat_cue_fabs=mssql_fetch_array($con_dat_cue_fabs))
{
	$lista_cuentas_fabs.=$res_dat_cue_fabs['dis_por_con_fab_cue_uno']."1,";
}
//echo $lista_cuentas_fabs;
$cuenta_fabs = substr($lista_cuentas_fabs, 0, -1);
*/

$sql = "EXECUTE FABS $doc_ini,$doc_ini,'$cuenta_fabs'";
//echo $sql;
...
if($query&&$res_num_filas['num_filas']>0)
{
    ...
    while($row = mssql_fetch_array($que_balance))
    {
      ...
      //AQUI ESTA EL SALDO INICIAL DEL FABS
        ...
        $sal__cue_ter_ano="SELECT ISNULL(SUM(mov_valor),0) mov_valor,mov_compro,mov_concepto,mov_nit_tercero,mov_cent_costo,mov_tipo,mov_documento,
        ...
        mov_mes_contable=9
        GROUP BY mov_compro,mov_concepto,mov_nit_tercero,mov_cent_costo,mov_tipo,mov_documento,
		mov_doc_numer,mov_mes_contable,mov_ano_contable";
...
else//NO TIENE MOVIMIENTOS, BUSCAMOS EL SALDO INICIAL PARA MOSTRARLO
{
    ...
    while($res_nit_id=mssql_fetch_array($eje_nit_id))
    {
        ...
        $sal__cue_ter_ano="SELECT ISNULL(SUM(mov_valor),0) mov_valor,mov_compro,mov_concepto,mov_nit_tercero,mov_cent_costo,mov_tipo,
